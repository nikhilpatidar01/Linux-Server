...
Here’s a cleaned-up, fully CentOS-compatible Markdown guide. All commands are preceded by a brief description, and at the end you’ll find a **Tools** section listing every command-line utility used.
...
> *On CentOS 8+, you may need to use `sudo dnf install -y squid*` instead.*
...
2. **Capture packets on your network interface**  
   ```bash
   sudo tcpdump -i enp0s8 -w /root/squid_traffic.pcap
   ```
3. **Read and analyze the captured pcap file**  
   ```bash
   sudo tcpdump -r /root/squid_traffic.pcap
   ```
