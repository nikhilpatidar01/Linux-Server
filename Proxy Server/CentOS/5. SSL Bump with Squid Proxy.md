...
1. **Generate a Self-Signed CA (Certificate Authority) Certificate for Squid**:
   ```bash
...
2. **Verify that the certificate files are created**:
   ```bash
...
1. **Create and initialize the SSL database** Squid uses for dynamic certificate generation:
   ```bash
...
2. **Verify Squid is running and listening**:
   ```bash
...
1. **Open necessary ports (Squid proxy and HTTP)**:
   ```bash
   sudo firewall-cmd --permanent --zone=public --add-port=3128/tcp
   sudo firewall-cmd --permanent --zone=public --add-port=80/tcp
   ```

2. **Enable masquerading (required for IP forwarding/NAT)**:
   ```bash
...
1. **Serve the Squid CA certificate over HTTP**:
...
2. **Clients can download it by accessing**:
   ```
...
2. **Click Import**, select `squidCA.crt`.
