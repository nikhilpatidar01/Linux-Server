...
```bash
gzip n.txt              # Compress file using gzip.
gunzip n.txt.gz         # Decompress gzip file.
```

## 🗂️ Archive with tar

```bash
tar -cvf nikhil.tar game document nikhil.txt  # Create tar archive.
tar -xvf nikhil.tar                           # Extract tar archive.
```

## 🔁 Pipes & Redirection

```bash
ls > file.txt              # Redirect output to a file (overwrite).
ls >> file.txt             # Redirect output to a file (append).
ls wrongfolder 2> error.txt  # Redirect errors to a file.
```
